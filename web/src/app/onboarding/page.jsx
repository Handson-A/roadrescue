...
export default function OnboardingPage() {
  ...
  // Explicit window client observation engine to keep layout nodes isolated
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-[#FFF8EA] text-[#1F1B10] lg:grid lg:grid-cols-2 select-none">
      ...
      {/* ================= DESKTOP SIDEBAR HERO VIEW (ONLY RENDERS ON LARGE SCREENS) ================= */}
      {!isMobile ? (
        <section key="desktop-hero" className="relative hidden overflow-hidden bg-[#1F1B10] lg:flex lg:min-h-screen lg:flex-col lg:justify-center p-12 xl:p-16 w-full animate-in fade-in duration-300">
          ...
          <div className="relative z-10 w-full max-w-xl xl:max-w-2xl mx-auto text-white flex flex-col gap-10">
            ...
            <div className="w-full">
              <div className="relative h-96 overflow-hidden rounded-4xl border border-white/10 bg-[linear-gradient(180deg,rgba(255,197,34,0.18),rgba(31,27,16,0.16)),linear-gradient(135deg,#5a3a12_0%,#1e1a12_100%)] shadow-2xl">
                    ...
                    <div className="flex items-center gap-4 text-white">
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-sm leading-relaxed text-white/80">
                          Real-time assistance tracking for vehicle breakdowns, punctures, and towing operations.
                        </p>
                      </div>
                    </div>
                  </div>
      ...
      {/* ================= RIGHT HALF: ONBOARDING INTERACTION PANELS ================= */}
      <section className="flex min-h-screen items-center justify-center bg-[#FFF8EA] px-6 py-10 sm:px-12 md:px-16 lg:h-full lg:px-12 xl:px-24">
        <div className="w-full max-w-xl lg:max-w-2xl space-y-8 sm:space-y-10">
          ...
          {/* Mobile Only Header Layout Context */}
          {isMobile && (
            <div key="mobile-header-group" className="space-y-6 animate-in fade-in duration-200">
              <div className="flex items-center justify-between border-b border-[#E0D5B7] pb-4">
              ...
              <div className="relative h-64 overflow-hidden rounded-3xl border border-[#D8CBA8] bg-[#1F1B10] shadow-md">
                ...
                <div className="absolute inset-x-4 bottom-4 rounded-2xl border border-white/10 bg-black/25 p-4 backdrop-blur-sm">
                  <div className="flex items-center gap-3 text-white">
                    <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-[#F5D108] text-[#1F1B10]">
                      <CarFront size={20} />
                    </span>
                    <div>
                      <p className="text-base font-black">Emergency Dispatch</p>
                      <p className="text-xs text-white/80">Connecting you to nearby assistance.</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="space-y-3">
            ...
            <p className="text-base leading-relaxed text-[#5E5440] sm:text-lg">
              Select your platform access classification path to continue.
            </p>
          </div>

          {/* Option Action Blocks with Secure Key Tracking IDs */}
            ...
            {roleCards.map((card) => {
              ...
              return (
                <Link
                  key={card.id}
                  href={card.href}
                  className="group flex items-center gap-5 rounded-2xl border border-[#D8CBA8] bg-white px-5 py-4 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md"
                >
          ...
          <div className="grid gap-4 grid-cols-1 sm:grid-cols-2">
            <Link
              href="/auth/login"
              className="inline-flex h-14 items-center justify-center gap-2 rounded-2xl bg-[#F5D108] px-6 text-base font-black text-[#1F1B10] shadow-md shadow-amber-500/10 transition-colors hover:bg-[#e5c300]"
            >
            ...
            <Link
              href="/auth/login"
              className="inline-flex h-14 items-center justify-center rounded-2xl border border-[#C8B98E] bg-white px-6 text-base font-bold text-[#1F1B10] hover:bg-slate-50 transition-colors"
            >
              Sign In
            </Link>
          </div>
          ...
          <div className="border-t border-[#E0D5B7] pt-6">
            <div className="grid gap-4 grid-cols-1 sm:grid-cols-2">
              <div className="flex items-center gap-3 rounded-2xl bg-white px-5 py-4 text-sm sm:text-base text-[#5E5440] shadow-sm border border-slate-200/40">
