...
// Instantiates client handler. If your key drops, it uses local system properties safely
...
export async function POST(request) {
  try {
    ...
    // Contextual system prompt engineering to keep the AI focused exclusively on mechanical aid
    ...
    if (!process.env.GEMINI_API_KEY) {
      return NextResponse.json({
        reply: `Pre-compiled Assessment: The symptoms for "${message}" resemble battery current drops or fuel system pressure losses. Please check terminal connections.`
      });
    }

    // Call Gemini Free Tier model engine layer
